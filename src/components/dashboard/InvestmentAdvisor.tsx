...
import { AlertTriangle, ArrowDown, ArrowRight, ArrowUp, BarChart3, PieChart, TrendingUp } from 'lucide-react';
import { useCallback, useEffect, useState } from 'react';

import { useSession } from '@/components/providers/SessionProvider';
import type { PortfolioAnalysis, PortfolioAllocation } from '@/lib/investment-advisor';
...
export function InvestmentAdvisor({ className = '' }: InvestmentAdvisorProps) {
  ...
  const [selectedTab, setSelectedTab] = useState<'overview' | 'allocations' | 'recommendations' | 'performance'>('overview');
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className="bg-gray-50 rounded-lg p-4">
                ...
                <div className={`inline-flex px-2 py-1 rounded-full text-sm font-medium ${getRiskColor(analysis.riskAssessment.riskLevel)}`}>
            ...
            {analysis.rebalancingNeeded && (
              <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                ...
                <p className="text-sm text-yellow-700">
                  Your portfolio has drifted from target allocations. Consider rebalancing by {analysis.nextRebalanceDate}.
                </p>
              </div>
            )}
          </div>
          ...
          <div className="space-y-4">
            ...
            {analysis.recommendations.length === 0 ? (
              ...
              analysis.recommendations.map((rec, index) => (
                <RecommendationCard key={index} recommendation={rec} getPriorityColor={getPriorityColor} />
              ))
            )}
          </div>
        )}
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div className="text-center">
                <div className="text-2xl font-bold text-gray-900">{analysis.performance.totalReturn}%</div>
                <div className="text-sm text-gray-600">Total Return</div>
              </div>
              <div className="text-center">
                <div className="text-2xl font-bold text-gray-900">{analysis.performance.ytdReturn}%</div>
                <div className="text-sm text-gray-600">YTD Return</div>
              </div>
              <div className="text-center">
                <div className="text-2xl font-bold text-gray-900">{analysis.performance.alpha}%</div>
                <div className="text-sm text-gray-600">Alpha</div>
              </div>
              <div className="text-center">
                <div className="text-2xl font-bold text-gray-900">{analysis.riskAssessment.sharpeRatio}</div>
            ...
            <div className="bg-gray-50 rounded-lg p-4">
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                ...
                <div>
                  ...
                  <span className="ml-2 font-medium">{analysis.riskAssessment.diversificationScore}%</span>
                </div>
                <div>
    ...
    <div className="border border-gray-200 rounded-lg p-4">
      ...
      <div className="space-y-2">
        ...
        <div className="flex justify-between text-sm">
          ...
          <span className={`font-medium ${allocation.variance > 0 ? 'text-red-600' : allocation.variance < 0 ? 'text-green-600' : 'text-gray-900'}`}>
        ...
        <div className="w-full bg-gray-200 rounded-full h-2">
          ...
          >
            <div 
              ...
              style={{ left: `${(allocation.targetPercentage / Math.max(allocation.currentPercentage, allocation.targetPercentage)) * 100}%` }}
            />
          </div>
        </div>
      </div>
  ...
  return (
    <div className="border border-gray-200 rounded-lg p-4">
      <div className="flex items-start justify-between mb-3">
        ...
        <span className={`inline-flex px-2 py-1 rounded-full text-xs font-medium border ${getPriorityColor(recommendation.priority)}`}>

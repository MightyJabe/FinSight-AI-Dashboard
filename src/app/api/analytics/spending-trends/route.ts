...
import { z } from 'zod';
import { formatISO, subMonths, subDays, format, startOfWeek, endOfWeek, eachDayOfInterval, eachMonthOfInterval, startOfMonth, endOfMonth } from 'date-fns';
...
const trendAnalysisSchema = z.object({
  ...
  analysisType: z.enum(['category', 'monthly', 'weekly', 'daily', 'seasonal', 'anomaly']).default('category'),
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const result: SpendingTrends = {
      ...
      averagePerPeriod: trends.length > 0 ? trends.reduce((sum, t) => sum + t.amount, 0) / trends.length : 0,
      trends,
      insights,
      projections
    };
 ...
 */
async function fetchUserTransactions(userId: string, startDate: Date, endDate: Date): Promise<Transaction[]> {
 ...
 */
async function fetchCategorizedTransactions(userId: string, startDate: Date, endDate: Date): Promise<Transaction[]> {
  const allTransactions: Transaction[] = [];
  ...
  const categorizedMap: Record<string, {
    aiCategory: string;
    aiConfidence: number;
    aiReasoning?: string;
    type: 'income' | 'expense';
  ...
  for (const transaction of rawTransactions) {
    ...
    if (categorizedData && categorizedData.type === 'expense') {
      ...
      allTransactions.push({
        ...
        aiReasoning: categorizedData.aiReasoning
      });
 ...
 */
function analyzeMonthlyTrends(transactions: Transaction[], startDate: Date, endDate: Date): TrendData[] {
 ...
 */
function analyzeWeeklyTrends(transactions: Transaction[], startDate: Date, endDate: Date): TrendData[] {
  const trends: TrendData[] = [];
 ...
 */
function analyzeDailyTrends(transactions: Transaction[], startDate: Date, endDate: Date): TrendData[] {
 ...
 */
function analyzeCategoryTrends(transactions: Transaction[], startDate: Date, endDate: Date): TrendData[] {
...
function analyzeSeasonalTrends(transactions: Transaction[]): TrendData[] {
  ...
  transactions.forEach(transaction => {
    ...
    const season = Object.keys(seasons).find(s => seasons[s as keyof typeof seasons].includes(month));
  ...
  dailySpending.forEach((amount, date) => {
    if (amount > threshold) {
      anomalies.push({
        ...
        amount,
        transactionCount: transactions.filter(t => format(new Date(t.date), 'yyyy-MM-dd') === date).length,
        anomaly: true
      });
    }
  });
 ...
 */
function generateInsights(trends: TrendData[], analysisType: string, _transactions: Transaction[]): string[] {
  ...
  switch (analysisType) {
    case 'monthly':
      ...
      if (monthlyGrowth.length > 0 && monthlyGrowth[0]) {
        insights.push(`Spending increased significantly in ${monthlyGrowth.length} month(s), with the highest increase of ${monthlyGrowth[0].percentChange?.toFixed(1)}% in ${monthlyGrowth[0].period}.`);
      }
      
      if (monthlyDecline.length > 0 && monthlyDecline[0]) {
        insights.push(`Spending decreased significantly in ${monthlyDecline.length} month(s), with the largest decrease of ${Math.abs(monthlyDecline[0].percentChange || 0).toFixed(1)}% in ${monthlyDecline[0].period}.`);
      }
      break;
    ...
    case 'category':
      ...
      insights.push(`Your highest spending category is "${topCategory.category}" accounting for ${topCategoryPercentage.toFixed(1)}% of total expenses.`);
      ...
      if (trends.length > 3) {
        ...
        insights.push(`Your top 3 categories account for ${topThreePercentage.toFixed(1)}% of all spending.`);
      }
      break;
    ...
    case 'seasonal':
      if (trends.length > 0) {
        ...
        insights.push(`You spend the most during ${maxSeason.period} and least during ${minSeason.period}.`);
      }
      break;

    case 'anomaly':
      if (trends.length > 0 && trends[0]) {
        insights.push(`Detected ${trends.length} unusual spending day(s). Your highest anomaly was $${trends[0].amount.toFixed(2)} on ${trends[0].period}.`);
      } else {
 ...
 */
function calculateProjections(trends: TrendData[], _analysisType: string): { nextPeriod: number; confidence: number; factors: string[] } {
  if (trends.length < 2) {
    return {
      nextPeriod: 0,
      confidence: 0,
  ...
  const variance = trendChanges.reduce((sum, c) => sum + Math.pow(c - avgChange, 2), 0) / trendChanges.length;

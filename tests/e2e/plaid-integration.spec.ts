...
test.describe('Plaid Integration', () => {
  ...
  test('should display account connection interface', async ({ page }) => {
    ...
    const connectButton = page.locator('button:has-text("Connect"), button:has-text("Add"), button:has-text("Link")');
  ...
  test('should open Plaid Link when connecting new account', async ({ page }) => {
    ...
    await page.addInitScript(() => {
      let plaidLinkOpened = false;
      
      window.Plaid = {
        create: (config: any) => ({
          open: () => {
            plaidLinkOpened = true;
  ...
  test('should handle account removal', async ({ page }) => {
    ...
    const removeButton = page.locator('button:has-text("Remove"), button:has-text("Disconnect"), button:has-text("Delete")');
  ...
  test('should handle Plaid Link errors gracefully', async ({ page }) => {
    ...
    if (await connectButton.isVisible()) {
      ...
      const errorMessage = page.locator(':has-text("error"), :has-text("failed"), .error');
  ...
  test('should refresh account data', async ({ page }) => {
    ...
    const refreshButton = page.locator('button:has-text("Refresh"), button:has-text("Update"), button[aria-label*="refresh"]');
